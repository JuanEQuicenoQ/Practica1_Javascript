const pokeCard = document.querySelector('[data-poke-card]');
const pokeName = document.querySelector('[data-poke-name]');
const pokeImg = document.querySelector('[data-poke-img]');
const pokeImgContainer = document.querySelector('[data-poke-img-container]');
const pokeId = document.querySelector('[data-poke-id]');
const pokeTypes = document.querySelector('[data-poke-types]');
const pokeStats = document.querySelector('[data-poke-stats]');
const errorMessage = document.getElementById('error-message');

const typeColors = {
    electric: '#FFEA70',
    normal: '#B09398',
    fire: '#FF675C',
    water: '#0596C7',
    ice: '#AFEAFD',
    rock: '#999799',
    flying: '#7AE7C7',
    grass: '#4A9681',
    psychic: '#FFC6D9',
    ghost: '#561D25',
    bug: '#A2FAA3',
    poison: '#795663',
    ground: '#D2B074',
    dragon: '#DA627D',
    steel: '#1D8A99',
    fighting: '#2F2F2F',
    default: '#2A1A1F',
};

let currentPokemon = null;

const searchPokemon = event => {
    event.preventDefault();
    const { value } = event.target.pokemon;

    fetch(`https://pokeapi.co/api/v2/pokemon/${value.toLowerCase()}`)
        .then(data => {
            if (!data.ok) {
                throw new Error(`Error ${data.status}: ${data.statusText}`);
            }
            return data.json();
        })
        .then(response => {
            const speciesUrl = response.species.url;
            fetch(speciesUrl)
                .then(speciesResponse => speciesResponse.json())
                .then(speciesInfo => {
                    const evolutionChainUrl = speciesInfo.evolution_chain.url;
                    fetch(evolutionChainUrl)
                        .then(evolutionChainResponse => evolutionChainResponse.json())
                        .then(evolutionChainInfo => {
                            const evolutions = getEvolutions(evolutionChainInfo.chain);
                            response.evolutions = evolutions;
                            renderPokemonData(response);
                            errorMessage.style.display = 'none'; 
                        })
                        .catch(err => {
                            console.error('Error al obtener información de la cadena de evolución:', err);
                            renderPokemonData(response);
                        });
                })
                .catch(err => {
                    console.error('Error al obtener información de especies:', err);
                    renderPokemonData(response);
                });
        })
        .catch(err => {
            console.error('Error al buscar el Pokémon:', err.message);
            renderNotFound();
        });
}

const getEvolutions = chain => {
    const evolutions = [];
    let currentPokemon = chain.species.name;

    while (chain.evolves_to.length > 0) {
        const nextEvolution = chain.evolves_to[0];
        evolutions.push(nextEvolution.species.name);
        chain = nextEvolution;
    }

    return evolutions;
}

const renderPokemonData = data => {
    const sprite = data.sprites.front_default;
    const { stats, types, evolutions } = data;

    pokeName.textContent = data.name;
    pokeImg.setAttribute('src', sprite);
    pokeId.textContent = `Nº ${data.id}`;
    setCardColor(types);
    renderPokemonTypes(types);
    renderPokemonStats(stats);
}

const setCardColor = types => {
    const colorOne = typeColors[types[0].type.name];
    const colorTwo = types[1] ? typeColors[types[1].type.name] : typeColors.default;
    pokeImg.style.background = `radial-gradient(${colorTwo} 33%, ${colorOne} 33%)`;
    pokeImg.style.backgroundSize = ' 5px 5px';
}

const renderPokemonTypes = types => {
    pokeTypes.innerHTML = '';
    types.forEach(type => {
        const typeTextElement = document.createElement("div");
        typeTextElement.style.color = typeColors[type.type.name];
        typeTextElement.textContent = type.type.name;
        pokeTypes.appendChild(typeTextElement);
    });
}

const renderPokemonStats = stats => {
    pokeStats.innerHTML = '';
    stats.forEach(stat => {
        const statElement = document.createElement("div");
        const statElementName = document.createElement("div");
        const statElementAmount = document.createElement("div");
        statElementName.textContent = stat.stat.name;
        statElementAmount.textContent = stat.base_stat;
        statElement.appendChild(statElementName);
        statElement.appendChild(statElementAmount);
        pokeStats.appendChild(statElement);
    });
}

const renderNotFound = () => {
    errorMessage.textContent = '¡Error! Pokémon no encontrado. Por favor, verifica el nombre o número del Pokémon.';
    errorMessage.style.display = 'block';
    setTimeout(() => {
        window.location.href = 'index.html';
    }, 3000);
}

const loadHome = () => {
    currentPokemon = null;
    pokeName.textContent = 'Pokedex';
    pokeImg.setAttribute('src', './poke-shadow.png');
    pokeImg.style.background = '#fff';
    pokeTypes.innerHTML = '';
    pokeStats.innerHTML = '';
    pokeId.textContent = '';
}
